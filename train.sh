...
# echo -e "${Green}Doing Chinese word segmentation${NC}"
# cat tmp/pre_raw.txt | python -m jieba -d > tmp/raw.txt

# echo -e "${Green}Transforming punctuations${NC}"
# ${PYTHON} ./src/preprocessing/punctuation.py -i tmp/raw.txt -o tmp/raw.cleaned.txt

# echo -e "${Green}Transforming to binary format${NC}"
# ./bin/from_raw_to_binary_text tmp/raw.cleaned.txt tmp/sentencesWithPunc.buf

# frequent phrase mining for phrase candidates
# echo -e "${Green}Mining frequent phrases as candidates${NC}"
# ${PYTHON} ./src/frequent_phrase_mining/main.py -thres ${SUPPORT_THRESHOLD} -o ./results/patterns.csv -raw tmp/raw.cleaned.txt
# ${PYTHON} ./src/preprocessing/compute_idf.py -raw tmp/raw.cleaned.txt -o results/wordIDF.txt
...
# ./bin/from_raw_to_binary tmp/raw.cleaned.txt tmp/sentences.buf
